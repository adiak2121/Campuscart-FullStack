...
import api from '../api';
import type { AuthResponse, Listing, Order } from '../types';
...
export default function AdminPage() {
  ...
  const adminAuth = JSON.parse(localStorage.getItem("campuscart-admin") || "{}") as Partial<AuthResponse>;
  const admin = adminAuth.user || { email: "", name: "Admin", role: "ADMIN" };

const loadListings = async () => {
  try {
    const response = await api.get<Listing[]>("/admin/listings");
    setListings(response.data);
  } catch {
    showToast("Failed to load admin listings. Please login again.", "error");
  }
};
...
const loadOrders = async () => {
  try {
    ...
    showToast("Failed to load admin orders. Please login again.", "error");
  }
};
...
const updateStatus = async (orderId: string, status: string) => {
  ...
  try {
    const response = await api.put<Order>(`/admin/order/${orderId}/status`, { status });
